...
def get_bundled_data_path():
    """Get the path to bundled data directory."""
    if sys.version_info >= (3, 9):
        import importlib.resources
        return str(importlib.resources.files("estimate_train_time.data"))
    else:
        import pkg_resources
        return pkg_resources.resource_filename("estimate_train_time", "data")


def get_bundled_regressors_path(cluster_name=None):
    """Get the path to bundled regressor data.

    Args:
        cluster_name: Optional cluster name (e.g., 'Perlmutter', 'Vista')

    Returns:
        Path to regressors directory or cluster-specific subdirectory
    """
    base_path = os.path.join(get_bundled_data_path(), "regressors")
    if cluster_name:
        return os.path.join(base_path, cluster_name)
    return base_path


def get_bundled_examples_path():
    """Get the path to bundled example configurations."""
    return os.path.join(get_bundled_data_path(), "examples")
