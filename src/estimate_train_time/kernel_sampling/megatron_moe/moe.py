...
from .mpu import get_expert_token_counts_for_rank
from .mpu import get_expert_tokens_for_rank
from .mpu import copy_to_expert_model_parallel_region
from .mpu import gather_from_expert_model_parallel_region
...
class ParallelDroplessMLP(torch.nn.Module):
    ...
    def forward(self, x, expert_weights, expert_indices):
        ...
        # restore input shape
        x = x.view(in_shape)
        # print(f'ParallelDroplessMLP output dtype: {type(x)}')
        # ParallelDroplessMLP output dtype: <class 'torch.Tensor'>
        return x
